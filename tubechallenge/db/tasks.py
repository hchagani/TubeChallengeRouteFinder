...
def fill_db():
    ...
    with db.SessionLocal() as session:
        ...
        # Tube lines
        line_infos = get_tube_lines()
        new_lines = line.create(line_infos=line_infos, session=session)
        if new_lines is None:
            graph.update(status=enums.StatusFlag.FAILED, session=session)
            return

        # Tube stations
        station_infos, branch_infos = get_tube_stations_and_branches(new_lines)
        ...
        run_connections_infos = get_running_connections(
            run_line=run_line[0], session=session
        )
        ...
        graph.update(status=enums.StatusFlag.COMPLETED, session=session)
        return
